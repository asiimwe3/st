import Link from 'next/link'

const PHONE = "256700000000"        // UPDATE
const PHONE_DISPLAY = "+256 700 000 000"
const EMAIL = "info@stpetersmedical.ug"

export default function Footer() {
  return (
    <footer className="bg-gray-900 text-white pt-16 pb-8">
      <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-4 gap-10">
        {/* Brand */}
        <div>
          <div className="flex items-center gap-3 mb-4">
            <div className="w-12 h-12 bg-primary rounded-full flex items-center justify-center font-bold text-lg">SP</div>
            <div>
              <p className="font-bold text-lg leading-tight">St. Peters</p>
              <p className="text-secondary text-sm">Medical Center</p>
            </div>
          </div>
          <p className="text-gray-400 text-sm mb-4">Providing compassionate, quality healthcare to the people of Kyenjojo and beyond.</p>
          <a href={`https://wa.me/${PHONE}`} target="_blank" rel="noreferrer"
            className="inline-flex items-center gap-2 bg-green-600 text-white text-sm px-4 py-2 rounded-full font-semibold hover:bg-green-700 transition">
            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/>
              <path d="M12 0C5.373 0 0 5.373 0 12c0 2.126.554 4.122 1.523 5.853L0 24l6.313-1.496A11.96 11.96 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22c-1.891 0-3.659-.5-5.19-1.374l-.373-.22-3.747.888.937-3.638-.243-.386A9.945 9.945 0 012 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10z"/>
            </svg>
            WhatsApp Us
          </a>
        </div>

        {/* Quick Links */}
        <div>
          <h4 className="font-semibold text-lg mb-4 text-secondary">Quick Links</h4>
          <ul className="space-y-2 text-gray-400 text-sm">
            <li><Link href="/" className="hover:text-white transition">Home</Link></li>
            <li><Link href="/about" className="hover:text-white transition">About Us</Link></li>
            <li><Link href="/services" className="hover:text-white transition">Our Services</Link></li>
            <li><Link href="/doctors" className="hover:text-white transition">Our Doctors</Link></li>
            <li><Link href="/appointment" className="hover:text-white transition">Book Appointment</Link></li>
            <li><Link href="/contact" className="hover:text-white transition">Contact Us</Link></li>
          </ul>
        </div>

        {/* Services */}
        <div>
          <h4 className="font-semibold text-lg mb-4 text-secondary">Our Services</h4>
          <ul className="space-y-2 text-gray-400 text-sm">
            <li>General Medicine</li>
            <li>Maternity & OB/GYN</li>
            <li>Pediatrics</li>
            <li>Laboratory</li>
            <li>Pharmacy</li>
            <li>Emergency Care (24/7)</li>
          </ul>
        </div>

        {/* Contact */}
        <div>
          <h4 className="font-semibold text-lg mb-4 text-secondary">Contact Us</h4>
          <ul className="space-y-3 text-gray-400 text-sm">
            <li className="flex items-start gap-2">
              <span>📍</span>
              <span>Kyenjojo Town, Kagadi Road, Uganda</span>
            </li>
            <li>
              <a href={`tel:${PHONE}`} className="flex items-center gap-2 hover:text-white transition">
                <span>📞</span>{PHONE_DISPLAY}
              </a>
            </li>
            <li>
              <a href={`mailto:${EMAIL}`} className="flex items-center gap-2 hover:text-white transition">
                <span>✉️</span>{EMAIL}
              </a>
            </li>
            <li className="flex items-start gap-2">
              <span>🕐</span>
              <span>Mon–Sat: 8:00AM – 6:00PM</span>
            </li>
            <li className="flex items-center gap-2 text-red-400 font-semibold">
              <span>🚨</span>Emergency: 24/7
            </li>
          </ul>
        </div>
      </div>

      <div className="max-w-7xl mx-auto px-4 mt-12 pt-6 border-t border-gray-700 flex flex-col md:flex-row justify-between text-gray-500 text-sm gap-2">
        <p>© 2026 St. Peters Medical Center. All rights reserved.</p>
        <p>Built with ❤️ for better healthcare in Uganda</p>
      </div>
    </footer>
  )
}
